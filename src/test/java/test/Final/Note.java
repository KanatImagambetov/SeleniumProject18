...
-> id, name, clasname, tagname, linktext, partialLinkText, cssSelector, xpath
...
- Not reliable, because it will easly break with any changes in HTML structure change.
2- Relative xpath:
- // starts with double slash
- it allows us to start anywhere in the html code, it is more reliable because we are starting from either the webelement itself OR one of the static parents or childs.
- // when we say // it will jump the the given web element
...
- Whenever you are having hard time locating with xpath, always try to find a static (stable/non changing) parent or child to the desired web element we are trying to locate. And go from there
...
- We just locate it as any other web element using one of the 8 locators of selenium. And click.
...
1- Warning/Information : We have only option to accept.
